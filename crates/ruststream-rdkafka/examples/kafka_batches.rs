...
// whole page uniformly. `workers(4)` keeps up to four pages in flight at once; `by_key` does not
...
#[subscriber(
    KafkaTopic::new("payments")
        ...
        // the descriptor's config passthrough; the page size is the mount site's word.
...
fn app() -> impl App {
    ...
    RustStream::new(AppInfo::new("orders", "0.1.0")).with_broker(broker, |b| {
        ...
        b.retry_via(retries);
        // --8<-- [start:size]
        // `batch(n)` is the page size, and a page handler does not mount without it: it travels
        // to the consumer's poll, which never hands the body more than `n` records at once.
        b.include(handle_page.batch(nonzero!(64)));
        b.include(reconcile_page.batch(nonzero!(16)));
        // --8<-- [end:size]
    })
}
// --8<-- [end:app]
