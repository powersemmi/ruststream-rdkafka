//! Live end-to-end for the Avro registry middleware: a publishing handler's plain-JSON
//! replies get framed as Avro by the app's `SchemaFrame` publish layer, and a plain JSON
//! handler consumes them back through the broker's transcoding subscription.
...
async fn live_avro_middleware_end_to_end() {
    const COUNT: usize = 3;
    ...
    // registry's compatibility check between runs).
    let subject = format!("avro-mw-{run}");
    let schema = format!(
        r#"{{"type":"record","name":"Order{base}","fields":[
            {{"name":"id","type":"long"}},{{"name":"item","type":"string"}}]}}"#,
    );
    ...
    let app = RustStream::new(AppInfo::new("avro-mw", "0.0.0"))
        ...
        .with_broker(broker, |b| {
            b.include(avro_mw);
            b.include(avro_relay).publisher(KafkaPublish::default());
        });
    ...
    assert_eq!(
        seen, expected,
        "Avro on the wire must arrive as JSON in plain handlers",
    );
    assert!(
