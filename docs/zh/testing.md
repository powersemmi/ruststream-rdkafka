...
## Protobuf 处理器 { #protobuf-handlers }

面对生成的 Protobuf 消息的处理器在这里也是普通处理器：类型自带字节，因此它完全不需要集群，而读一条
带信封的投递连注册表也不需要。`TestApp` 的写法和它的手写版对应形态，参见
[Schema Registry 这一页](schema-registry.md#testing-a-protobuf-handler)。
...
- **分区。** 每个主题恰好一个分区，编号为零。`Ctx<Partition>` 永远读到 `0`，`partition(..)` 步骤
  改变不了记录的落点，定位到任何别的分区会被拒绝，而不是凭空造一个出来。这项设置仍然被记录下来，
  因此 `with_options(..)` 能读回调用点或变换选的值。工作分区*是*重现了的：一条订阅的 `LaneKey` 在这里的解析方式与上游完全一致，因此在默认的
...
- **已提交的位置活不过它的订阅。** 位置存在于订阅上，因此一次“重启”什么都留不下：同一个消费者组里
  ...
  重启后续读这件事要在集群上测。
- **重试与死信在这里是真跑的**，因为做这件事的是框架而不是消费者：注册声明的 `max_attempts(..)`
  和 `dead_letter(..)` 在替身上同样生效，`retry_after` 会延迟它的副本，`tb.advance(..)` 把它带回
  来。不同的是底下的重新投递。没有被声明覆盖的 `nack(true)` 在这里直接把投递交回来，而集群会让偏
  移量保持未结算，只在下一次拉取这个分区时才回到它。
