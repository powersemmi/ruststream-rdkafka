...
- Подтверждение идёт по зафиксированной позиции Kafka, а не по кадру отдельного сообщения. Режим
...
превращает ту в политику на раздел, а `KafkaEosPublish` - политика конвейера exactly-once.
...
| `OwnedTransactions` | нет | Продюсер Kafka держит одновременно одну транзакцию на стороне брокера, поэтому транзакция не может быть отдельным значением во владении; параллельные потоки берут [издателей на раздел](publishing.md#transaction-scopes-and-worker-pools) или [конвейер exactly-once](publishing.md#exactly-once-pipelines). |
...
| `Partitioned` | да | Упорядоченные рабочие дорожки с ключом по исходному разделу доставки или по ключу записи при `LaneKey::RecordKey`: [Рабочие дорожки по ключу](topics.md#keyed-worker-lanes). |
| `Seekable` + `Positioned` | да | Перемещайте разделы, которыми владеет этот консьюмер, прямо из обработчика - через ключ контекста `SeekHandle` рядом с `Position` самой доставки: [Перемотка подписки](topics.md#repositioning-a-subscription). |
...
- [Темы и группы](topics.md) - дескрипторы, стартовые смещения, режимы фиксации, дорожки по ключу.
